...
- 審査結果チャット: `pages/10_examination.py` にチャットパネルを追加。コンテキストは対象条文/審査結果/ナレッジ/契約基本情報を添付。履歴はセッション内保持。新たな審査開始をしたときに履歴をリセットする。
- ナレッジ生成: 新規ページ `pages/22_knowledge_llm.py`（案）で対話→下書きナレッジを編集→`KnowledgeAPI.save_knowledge` へ保存。下書き保存は `approval_status=draft` で登録。
- 《これはキャンセル》ログ/監査: 重要操作（保存/削除/生成リクエスト）を専用コンテナ or ログファイルに記録する案を検討。機密情報はサニタイズ。
- 権限制御: 生成/保存は管理者のみ。閲覧チャットは一般でも可とするか要検討。

## 技術タスク（ドラフト）
- UI: 審査結果チャットパネル追加（チャット履歴、入力、送信、モデル選択、コンテキスト表示切替）。Streamlitは `st.columns([3,2,3])` で「審査結果」「ナレッジ」「チャット」を横並び、チャット列に `st.chat_message`/`st.chat_input`。幅が足りない場合は `st.tabs(["結果","ナレッジ","チャット"])` へフォールバックし、サイドバーに簡易サマリを置く。
- UI: サイドバー内チャット可否デモは一時ファイルで検証済み（削除済）。`st.sidebar` 直下に `st.chat_input`/`st.chat_message` を置く構成で実装可能と確認。`pages/10_examination.py` に実装済み（対象条文/審査結果/ナレッジ/契約基本情報をそのままコンテキストに添付）。
