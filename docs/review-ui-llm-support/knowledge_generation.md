...
## ナレッジ生成タブ（LLMモック）
- 入力: 契約タイトル/種別/当事者/背景、対象条文番号+本文、参考ナレッジ追加入力。
- 選択済みナレッジ: 前タブの選択結果をJSONで表示（コピー用）。Userプロンプトにも埋め込み。
- プロンプトプレビュー: 下記SystemとUserテンプレをJSON埋め込みで表示。
- モック出力: ボタンで下記フォーマットに沿ったJSONを生成表示。LLM呼び出しなし、必須項目が空のままでもそのまま返す。
